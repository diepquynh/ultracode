...
{{tool_read}} `{{skills_dir}}/module-hub/references/*.md` to learn the house structure. For UPDATE, read the
...
`{{skills_dir}}/module-hub/references/` are still written with {{tool_write}} or {{tool_edit}} as in Step 5.
```markdown
...
Return plain text: the report path, a one-sentence summary of what was created or updated, and the list of
...
8. Commands from the profile. Run only the repo profile's `commands.format` string verbatim. Never hardcode a
   build tool.
