...
description: >
  ...
  modified, or deleted to fulfill a request, (3) the user asks to "implement", "build", "code", "create",
  "add", "modify", "fix", "refactor", or "change" something, (4) code-reviewer findings must be fixed (the
  orchestrator re-spawns this agent with fix instructions), (5) verification failures need code changes.
...
| **session dir** | Scratch directory from the prompt's `Session dir:`. All output files go here. Already exists — do not mkdir. **If the prompt omits it,** derive it: `{repo-root}/.claude/ultracode/session/ultracode-session-${CLAUDE_CODE_SESSION_ID:-no-session-id}`. You inherit `CLAUDE_CODE_SESSION_ID` from the orchestrator unchanged, so that resolves to the same dir every other agent in this session uses; `mkdir -p` it in that case (a no-op if it exists). Never invent a random or timestamped dir name — the code-reviewer, EPA, and write-test agents read your change report from this exact path, and your progress log must be found by a re-spawn. |
