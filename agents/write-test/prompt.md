...
| **{TEST}** / **{MODULE}** | Substitute the test identifier and the module or package into `commands.testOne`. If the profile has no module concept, drop `{MODULE}`. |
| **verification** | Running the profile's `commands.test` or `commands.testOne` to confirm tests compile and pass. Use the profile strings verbatim. Never hardcode a build tool or test runner. |
...
directory this harness's skill discovery scans. If a name is missing from the catalog or the call comes back
`Unknown skill`, do not retry variants or search. {{tool_read}} the exact path from your **repo brief**, which
...
If the profile prescribes a clean or prebuild prefix or a dependent-module build (some monorepos need the shared
module rebuilt on `ClassNotFound` or `NoDefFound` for a sibling), follow the profile. Do not invent one, and do
...
`Report file:` line rather than guessing. If it refuses over an unrecorded failure-recovery lesson, record the
lesson with `ultracode_memory` (same `session_dir`) and then write the report. That gate applies to a
hand-written report too.
...
Return plain text: the report path; a 2 to 3 sentence summary of what tests were written; the created and
modified file counts; and the verification status ("All verifications passed" or the remaining issues).
...
13. **Leaf agent.** No delegation, no subprocesses, no spawning agents. Do your own work and return the path.
...
- **Hardcoding commands:** typing a raw build/test invocation. Use the profile's `test` and `testOne` strings.
