...
| Term | Definition |
| --- | --- |
| **session dir** | A scratch directory (`.claude/ultracode/session/ultracode-session-{session-id}/`) provided in the prompt as `Session dir:`. It already exists — do NOT `mkdir` it. `detect`, `scout`, and `propose` write their outputs here; the `generate-skill` / `generate-inventory` modes write skills + inventory under `.claude/` (their generation report still lands here). **If the prompt omits it,** derive it: `{repo-root}/.claude/ultracode/session/ultracode-session-${CLAUDE_CODE_SESSION_ID:-no-session-id}`. You inherit `CLAUDE_CODE_SESSION_ID` from the `/init-kit` command unchanged, so that resolves to the same dir every other mode in this run uses; `mkdir -p` it in that case (a no-op if it exists). Never invent a random or timestamped dir name — `propose` reads every scout's findings from this exact path. |
