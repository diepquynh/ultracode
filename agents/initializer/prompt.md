...
| **legacy runtime dir** | A per-harness runtime dir from an older ultracode layout: a harness state dir plus `/ultracode` (for example the claude state dir's `ultracode` subdirectory). Ultracode no longer writes there. The runtime dir is now the shared `{{runtime_dir}}/`, but a repo bootstrapped before the move still carries one. |
| **cross-harness candidate** | A COMPLETE prior bootstrap (`repo-profile.json` plus `INVENTORY.md`) found in a **legacy runtime dir** while the shared `{{runtime_dir}}/` has no bootstrap of its own. Discovered read-only by Step D0, before any repo scan. Adopting one migrates its skills and inventory into `{{runtime_dir}}/` and this harness's `{{skills_dir}}/` instead of re-scouting. |
...
| **disposition** | A per-skill action set by the orchestrator from the user's approval-gate choice: `generate` (write a new skill), `regenerate` (overwrite an existing skill with a fresh one), or `reuse` (keep the existing skill unchanged and register it only). |
...
it inside each harness's own state dir. A repo bootstrapped back then already did the expensive part (scouting,
exemplar capture, skill authoring) but stored it where this run will not look. Skill bodies and the module map
are prose, not harness-specific, so such a bootstrap is worth migrating instead of re-scouting. Before spending
a full scan on this repo, check for one.
...
**If one or more valid candidates:** skip Steps D1 to D6 (do not scan the repo). {{tool_write}}
...
confirms that one or opts for a full scan instead.** Once the user decides, the main loop either spawns
`Mode: adopt` with the chosen candidate, or re-spawns `Mode: detect` with `Skip cross-harness check: yes` to
fall through to a normal scan. Do not write a scout plan in this branch. `generate-inventory`'s report filename
is reused by `adopt` instead (see Step A6).
...
From the stack reference and the actual files, determine the concrete build, test, test-one, format, lint, typecheck, and run commands (for example read `package.json` scripts, detect `./mvnw` vs `mvn`, `Makefile` targets, `pytest`/`tox`). Record the exact strings. Use `null` for any that do not exist.

### Step D5: Discover existing skills

The target repo may already carry skills under `{repo-root}/{{skills_dir}}/`, written by a prior init-kit run
or hand-authored by the team. Find every one so the `propose` mode can re-use it instead of regenerating:
...
1. The skill's `name` is its parent directory name (`{{skills_dir}}/entity/SKILL.md` gives `entity`).
...
4. Record the skill's path relative to the repo root: `{{skills_dir}}/{name}/SKILL.md` (this matches the path form the profile uses, not the absolute path `find` prints).
...
**Return:** the scout-plan path, the stack, the chosen reference path, the structured slice list (descriptor,
paths, slug), and the count of existing skills discovered in Step D5. The main loop reads this scout plan and
fans one scout out per slice.
...
the seeded defaults. This mode writes to the target repo, like `generate-skill` and `generate-inventory`. It is
not read-only.
...
for the exact `models` schema and its seeded defaults (`models.byAgent` and `models.byPhaseComplexity`). Do not
reuse whatever `models` block the source harness had.
...
1. Every path string that starts with the source's skills dir or runtime dir (for example `skills[].path`,
   Module/Area map `Reference` cells, the header's "Machine profile" link) is rewritten to the equivalent
   relative path under `{{skills_dir}}` or `{{runtime_dir}}`.
2. The profile's `models` block is replaced ENTIRELY with the seeded defaults read in Step A2. Never carry it
   over from the source. This is a hard reset, not a merge. Any per-repo model customization the source harness
   had (including a harness-specific object override) is discarded.
...
byte for byte. No translation is needed.
...
defaults exactly; every skill file listed in the profile exists on disk at its new path. Fix any mismatch by
editing.
...
{{tool_write}} `{session-dir}/ultracode-generate-report.md` (the same filename the normal pipeline's
...
For each component type with at least 1 match:
...
- **Convention skill**: recommend exactly one, distilled from conventions observed consistently across exemplars (naming, immutability keywords, timestamp handling, error and exception style, logging).
...
- **module-hub**: fill Archetype C from the proposal's module map. {{tool_write}} `{repo}/{{skills_dir}}/module-hub/SKILL.md` with the routing tables (path glob to area, area to reference). {{tool_write}} `{repo}/{{skills_dir}}/module-hub/references/{area}.md` only for an area complex enough to warrant it, grounded in real source.
...
Do not add `implementer`, `write-test`, or `initializer` to `byAgent`. The first two are
tier-driven. The initializer is spawned by the /init-kit command, which sets its model per mode, so leaving it
out does not break re-initialization. A route may be `"default"` to use the agent definition's neutral default or
