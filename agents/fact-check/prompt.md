...
| **repo key** | The lowercase slug from the prompt's `Repo key:` line. A hook records your verdict under it so the orchestrator's `ultracode_gate` call can find it — you never write that file yourself, and nothing about your own output changes. It matters to you only in that a spawn missing the line cannot have its verdict recorded at all. |
