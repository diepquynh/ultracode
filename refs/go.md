...
## Detection signals
- `go.mod` at root (module path) + dominant `.go` extension. `go.work` means a multi-module workspace.
...
- capture: three-file split. A bare interface in `service/`, a `Default*` struct in `service/impl/` with a `New*` constructor,
...
- capture: interface + `Default*Repository` struct holding two `*gorm.DB` (`readWriteDB`, `readOnlyDB`); `Configure*Repository` takes a
  ...
  `errors.Is(err, gorm.ErrRecordNotFound)` means `return nil, nil`; struct-based `Where(&entity.X{...})`; `Clauses(clause.Locking{Strength:"UPDATE"})` for row locks;
...
- capture: string-backed named type with a triplet of `const` per value (`XEnum` typed / `X` int / `XString`); `Scan` maps stored `int64` to string,
  `Value` maps string to int (DB stores SMALLINT, Go holds the string); unknown value means a wrapped error. New enum member = add all three consts + both switch arms.
...
- capture: fiber middleware registered in `main.go` via `Invoke` (sequence: `requestid`, `recover`, CORS, JWT); JWT via a fiber JWT contrib
  with a `Filter` that returns `true` only for whitelisted `path+method` (a map `EndpointsWhitelist`), so auth is default-on, opt-out per route;
