...
`{{plugin_root}}/skills/meta-author/SKILL.md` (16 Laws, Chain-of-Thought, self-review).

---

## Archetype A: Creation skill

Use for a recurring component type (entity, DTO, repository, service, controller, route, model, migration,
handler, and so on). Ground every section in the captured exemplar and invariants.
...
### 2. {Handle the invariant that needs extra steps, such as registration, wiring, or migration}
...
context. The template must compile or parse after substitution. Order steps by dependency (bottom-up).
...
`*Test`, `*_test`, or `*.spec` file). When the repo has **no tests yet**, ground the skill in the stack
reference's test convention and mark it `convention-seeded`. It then establishes the pattern that fills the T1
gap ("new public function without a test") rather than mirroring an existing convention.
...
Location: `{observed test-file convention}`, for example co-located `foo_test.go`, mirrored `tests/<domain>/test_<module>.py`, `src/test/java/.../{X}Test.java`, or co-located `*.spec.ts`.
...
Assert the return value, response body, or rendered state, then verify collaborators were (or were NOT)
...
- [ ] Every collaborator replaced with the stack's test double. No real network, DB, or queue in a unit test.
...
**Rules:** ground every template line in a real test exemplar. When the repo has no tests, ground in the stack
reference's test convention and mark the skill `convention-seeded`. One execution path per test. Never assert
framework internals.
