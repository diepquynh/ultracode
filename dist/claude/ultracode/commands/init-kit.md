...
(`sonnet`), but its modes differ in stakes, so set the `model` argument explicitly on every spawn
...
model (`opus`) — skill authoring is the highest-value, quality-sensitive step, so it gets the
...
**The path is derived, not generated.** The harness supplies `CLAUDE_CODE_SESSION_ID`, else `GROK_SESSION_ID`, which every agent inherits
...
1. Which files were written (per-skill SKILL.md files, INVENTORY.md, repo-profile.json), which existing skills
   ...
   skipped.
2. That newly generated skills register on the next session — advise running `/reload-plugins` or restarting the session so
   `.claude/skills/*` are discovered. (The INVENTORY.md routing works immediately because
