...
name: epa
description: >
  Spawn ultracode:execution-path-analyzer directly — enumerate every execution path through the
  changed code so tests can cover one path each.
---
...
**Spawn the prefixed name** — `agent_type: ultracode:execution-path-analyzer`, verbatim.
...
- **A report path in `the user's text following the explicit skill invocation`** → use it; its directory is the session dir.
...
```
agent_type: ultracode:execution-path-analyzer
