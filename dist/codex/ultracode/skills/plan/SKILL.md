...
name: plan
description: >
  Spawn ultracode:plan directly — turn the approved spec file into a phased implementation plan with
  per-phase files.
---
...
**Spawn the prefixed name** — `agent_type: ultracode:plan`, verbatim. A bare `plan` resolves to the
...
- **A spec path in `the user's text following the explicit skill invocation`** → use it; its directory is the session dir.
...
Read `$REPO_ROOT/.codex/ultracode/repo-profile.json`; take the model from `models.byAgent["plan"]` (bare key).
Absent → omit the `model` argument.

## Step 2 — Spawn

```
agent_type: ultracode:plan
