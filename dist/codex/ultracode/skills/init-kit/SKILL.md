...
name: init-kit
description: >
  Scout this repo for common coding patterns, propose a skill set for approval, then generate
  per-repo skills + a routing inventory — fanned out with parallel subagent spawns.
---
...
approval gate** — you spawn `ultracode:initializer` directly with the **spawn_agent tool**. Where the work is
independent, spawn in parallel: emit multiple spawn_agent tool calls in a **single message** and they run concurrently.

**Spawn the prefixed name.** Every spawn below passes `agent_type: ultracode:initializer` verbatim — the
...
**Re-using existing skills.** The repo may already carry skills under `.agents/skills/` (a prior init-kit run
...
(`gpt-5.6-terra`), but its modes differ in stakes, so set the `model` argument explicitly on every spawn
...
model (`gpt-5.6-sol`) — skill authoring is the highest-value, quality-sensitive step, so it gets the
...
Extra user focus for this run (may be empty): `the user's text following the explicit skill invocation`
...
ULTRACODE_SESSION="$SESSION_ROOT/ultracode-session-${CODEX_THREAD_ID:-no-session-id}"
...
**The path is derived, not generated.** The harness supplies `CODEX_THREAD_ID`, falling back to `no-session-id`, which every agent inherits
...
```
agent_type: ultracode:initializer
model: gpt-5.6-terra
prompt: "Mode: detect.
Repo root: {absolute repo root}.
User focus: the user's text following the explicit skill invocation
...
under {absolute repo root}/.agents/skills/ and record it in the scout plan's Existing Skills table (name, kind
...
`.agents/skills/{name}/` directory (disjoint paths), so the parallel fan-out needs no worktree isolation. Skip
...
Session dir: {ULTRACODE_SESSION}.
Repo root: {absolute repo root}.
Write ONLY your one skill into {absolute repo root}/.agents/skills/{skill.name}/ , grounded in this component
...
Session dir: {ULTRACODE_SESSION}.
Repo root: {absolute repo root}.
Write {absolute repo root}/.codex/ultracode/INVENTORY.md and {absolute repo root}/.codex/ultracode/repo-profile.json.
...
1. Which files were written (per-skill SKILL.md files, INVENTORY.md, repo-profile.json), which existing skills
   ...
   skipped.
2. That newly generated skills register on the next session — advise starting a new Codex session so
   `.agents/skills/*` are discovered. (The INVENTORY.md routing works immediately because
   agents read it as a file.)
3. That subsequent work in this repo will now route through `.codex/ultracode/INVENTORY.md`.
