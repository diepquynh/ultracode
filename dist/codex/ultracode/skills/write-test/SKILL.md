...
name: write-test
description: >
  Spawn ultracode:write-test directly — write one test per NEW execution path from the EPA report,
  following this repo's test skills.
---
...
**Spawn the prefixed name** — `agent_type: ultracode:write-test`, verbatim.
...
- **An EPA path in `the user's text following the explicit skill invocation`** → use it, plus the implement report it names.
...
```
agent_type: ultracode:write-test
