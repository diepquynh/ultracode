...
name: prompt-gen
description: >
  Spawn ultracode:prompt-generation directly — author or edit a prompt, SKILL.md, or subagent file
  against the meta-author standard.
---
...
- `Task` means the Codex `spawn_agent` capability.
...
**Spawn the prefixed name** — `agent_type: ultracode:prompt-generation`, verbatim.

Task: `the user's text following the explicit skill invocation`
...
The path is derived from the harness's session ID (`CODEX_THREAD_ID`, falling back to `no-session-id`), which
...
Resolve the **target**: an explicit path in `the user's text following the explicit skill invocation`, the file the task names, or `New` when the agent is
creating one. If the target exists, note its path so the agent edits rather than rewrites blindly.

Read `$REPO_ROOT/.codex/ultracode/repo-profile.json`; take the model from
...
```
agent_type: ultracode:prompt-generation
model: {models.byAgent["prompt-generation"], or omit}
prompt: "Repo root: {REPO_ROOT}.
Session dir: {SESSION_DIR}.
Target: {absolute target path, or 'New'}.
Task: {the request from the user's text following the explicit skill invocation}.
