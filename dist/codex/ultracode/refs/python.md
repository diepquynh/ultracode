...
- Package manager by lockfile: `uv.lock`→uv, `poetry.lock`→poetry, `Pipfile.lock`→pipenv, else pip/venv.
  `.python-version` pins the interpreter.
- Web framework: `fastapi`+`APIRouter` → FastAPI; `manage.py`+`django` → Django; `flask` → Flask; none of
  these + a `[project.scripts]`/`main()` → worker or CLI (may also be an AWS Lambda: `mangum` for HTTP, a
...
- capture: a `Protocol` declares the contract (a kind/source property + an `async` scrape method); each site impl satisfies it structurally, is decorated `@singleton @inject` with constructor DI of the fetch/proxy client; **conservative contract** — on any failure/parse-mismatch/empty body it logs and returns `None`, never raises; a bounded retry loop (`for attempt in range(1, MAX_ATTEMPTS+1)`) wraps the fetch; module-level `MAX_*`/selector/regex constants; query strings sometimes built by hand (not `urlencode`) to preserve param order/encoding.
...
- capture: a base `BusinessException(Exception)` carrying `status_code`/`error_code`/message; a pydantic error-response model (stable `code`/`message`/`status` keys); async handlers registered on the app (business → 4xx, catch-all → 500) as the global-advice analog; feature code raises the domain exception, never writes status inline.
...
- **Scraping is brittle by design**: selectors, regex on embedded JSON, tz-abbreviation offset maps, and
