...
# `dist/` is a build output, generated by install.sh from the checkout and never committed. So the tests
# generate their own plugin roots from a throwaway copy of the source tree, exactly as an install does.
IGNORED_SOURCE_ENTRIES = (".git", ".claude", ".codex", ".code-review-graph", "dist", "tmp", "sandbox")
GENERATED_SOURCE_ROOT = Path()
CLAUDE_PLUGIN_ROOT = Path()
CODEX_PLUGIN_ROOT = Path()
GENERATOR_STDOUT: dict[str, str] = {}
...
def run_generator_with_default_output(
    target: str, check: bool = False, source_root: Path | None = None
) -> subprocess.CompletedProcess[str]:
    command = [
        sys.executable,
        str(GENERATOR),
...
def setUpModule() -> None:
    """Build both plugin distributions the way install.sh does: from a clean copy of the sources.

    The generator's default output path is used on purpose — that `dist/<target>/ultracode` layout is
    the one install.sh points each harness marketplace at.
    """
    global _WORKSPACE, GENERATED_SOURCE_ROOT, CLAUDE_PLUGIN_ROOT, CODEX_PLUGIN_ROOT
    _WORKSPACE = tempfile.TemporaryDirectory()
    GENERATED_SOURCE_ROOT = Path(_WORKSPACE.name).resolve() / "checkout"
    shutil.copytree(
        ROOT,
        GENERATED_SOURCE_ROOT,
        ignore=shutil.ignore_patterns(*IGNORED_SOURCE_ENTRIES),
    )
    for target in ("claude", "codex"):
        result = run_generator_with_default_output(target, source_root=GENERATED_SOURCE_ROOT)
        if result.returncode != 0:
            raise AssertionError(f"generating the {target} plugin failed: {result.stderr}")
        GENERATOR_STDOUT[target] = result.stdout
    CLAUDE_PLUGIN_ROOT = GENERATED_SOURCE_ROOT / "dist" / "claude" / "ultracode"
    CODEX_PLUGIN_ROOT = GENERATED_SOURCE_ROOT / "dist" / "codex" / "ultracode"
...
class DefinitionTests(unittest.TestCase):
    ...
    def test_generated_output_passes_check_mode(self) -> None:
        for target, root in (
            ("claude", CLAUDE_PLUGIN_ROOT),
            ("codex", CODEX_PLUGIN_ROOT),
        ):
            with self.subTest(target=target):
                result = run_generator(
                    target, root, check=True, source_root=GENERATED_SOURCE_ROOT
                )
                self.assertEqual(result.returncode, 0, result.stderr)

    def test_check_mode_detects_stale_output(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            output = Path(temp_dir) / "ultracode"
            self.assertEqual(run_generator("claude", output).returncode, 0)
            stale = output / "skills" / "orchestrate" / "SKILL.md"
            stale.write_text(stale.read_text(encoding="utf-8") + "drift\n", encoding="utf-8")
            result = run_generator("claude", output, check=True)
    ...
    def test_default_output_uses_nested_harness_plugin_root(self) -> None:
        for target, expected_root in (
            ...
            with self.subTest(target=target):
                self.assertEqual(
                    expected_root, GENERATED_SOURCE_ROOT / "dist" / target / "ultracode"
                )
                self.assertIn(str(expected_root), GENERATOR_STDOUT[target])
                self.assertTrue((expected_root / "hooks" / "hooks.json").is_file())

    @unittest.skipUnless((ROOT / ".git").exists(), "not a git checkout")
    def test_distributions_are_generated_not_committed(self) -> None:
        ...
        self.assertEqual(tracked.stdout.strip(), "", "dist/ must never be committed")
        self.assertIn("/dist/", (ROOT / ".gitignore").read_text(encoding="utf-8"))

    def test_installer_generates_each_plugin_root_from_the_checkout(self) -> None:
        script = INSTALLER.read_text(encoding="utf-8")
        self.assertIn('PLUGIN_ROOT="$INSTALL_DIR/dist/$HARNESS/ultracode"', script)
        self.assertIn('rm -rf "$PLUGIN_ROOT"', script)
        self.assertIn(
            'python3 "$GENERATOR" --target "$HARNESS" --source-root "$INSTALL_DIR"'
            ' --output-dir "$PLUGIN_ROOT"',
            script,
        )
    ...
    def test_installer_dry_run_covers_both_harnesses(self) -> None:
        for target in ("claude", "codex"):
            with self.subTest(target=target):
                ...
                self.assertIn("Would generate dist/<harness>/ultracode", result.stdout)
    ...
    def test_model_router_keeps_the_initializer_model_when_reinitializing(self) -> None:
        for target, explicit_tier_model in (("claude", "haiku"), ("codex", "gpt-5.6-luna")):
            with self.subTest(target=target), tempfile.TemporaryDirectory() as temp_dir:
                repo = Path(temp_dir)
                runtime_dir = HARNESS_LAYOUT["layouts"][target]["runtime_dir"]
                profile_path = repo / runtime_dir / "repo-profile.json"
                profile_path.parent.mkdir(parents=True)
                # The seeded profile deliberately carries no initializer route, because init-kit picks
                # that model per mode. Re-running init-kit on an initialized repo must therefore keep
                # the spawn's own model instead of being denied like a missing pipeline route.
                profile = {
                ...
                self.assertEqual(output["updatedInput"]["model"], "chosen-by-init-kit")

                profile["models"]["byAgent"]["initializer"] = "fast"
                profile_path.write_text(json.dumps(profile), encoding="utf-8")
                overridden = route()
                self.assertEqual(overridden["updatedInput"]["model"], explicit_tier_model)

    def test_model_router_denies_a_malformed_profile(self) -> None:
        for content in ("not json", "[]"):
            with self.subTest(profile=content), tempfile.TemporaryDirectory() as temp_dir:
                repo = Path(temp_dir)
                runtime_dir = HARNESS_LAYOUT["layouts"]["claude"]["runtime_dir"]
                profile_path = repo / runtime_dir / "repo-profile.json"
                profile_path.parent.mkdir(parents=True)
                profile_path.write_text(content, encoding="utf-8")
                result = subprocess.run(
                    [sys.executable, str(CLAUDE_PLUGIN_ROOT / "hooks" / "model-router.py")],
                    input=json.dumps(
                        {
                            "cwd": str(repo),
                            "tool_input": {
                ...
                self.assertEqual(denied["permissionDecision"], "deny")
                self.assertIn("is invalid", denied["permissionDecisionReason"])
