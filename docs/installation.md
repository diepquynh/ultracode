...
The installer requires Git, Node 22.5 or newer for Ultracode's runtime hooks (the repo-memory store uses
`node:sqlite`), and the CLI for each selected harness.
...
From a checkout, use `bash install.sh` for all harnesses or pass `claude`, `grok`, `codex`, or `antigravity`
for one. The script keeps an updatable checkout under `${XDG_DATA_HOME:-$HOME/.local/share}/ultracode`.
Override that location with `ULTRACODE_INSTALL_DIR`. Pass `--dry-run` when running the local script to
preview what it would do. `all` (the default) includes Claude, Grok, Codex, and Antigravity.

What an install run does:

```mermaid
flowchart LR
    RUN["install.sh<br/>claude · grok · codex ·<br/>antigravity · all"] --> CHECKOUT["pull or keep the checkout under<br/>~/.local/share/ultracode<br/>(XDG_DATA_HOME · ULTRACODE_INSTALL_DIR)"]
...
| Codex | Start a new session, open `/hooks`, and trust Ultracode's hooks. Then start one more session so they take effect. Codex does not trust plugin hooks automatically, so the model-routing and pipeline guard hooks stay inactive until you do this. |
...
or disable one copy so skills and hooks do not fire twice.
...
`ultracode-gate` server is `mcp/hub-shim.js`, which ensures the hub at boot) or until you run the ctl command
yourself. Everything else (gate, report, memory) works without the hub. Set `ULTRACODE_HUB_DISABLE=1` in a
harness's environment to keep that machine or session free of the daemon.

## Uninstall

Reverts a matching `install.sh` run. The argument list is the same. Omit it (or pass `all`) to unregister every
harness and remove the checkout. Pass `claude`, `grok`, `codex`, or `antigravity` to unregister one and leave
the checkout for the remaining targets. `ULTRACODE_INSTALL_DIR` must match the install.
...
For active development, regenerate and load the plugin for one session without installing:
...
For a one-session run without installing, pass `--plugin-dir`:
...
Codex installs plugins from marketplaces and has no `--plugin-dir` option. Generate the distribution with
`node scripts/generate_definitions.js --target codex`, then stage `dist/codex/ultracode` beneath a local
marketplace, with this manifest at `<marketplace>/.agents/plugins/marketplace.json`:
...
plugins work in the CLI and in the Codex surface of the ChatGPT desktop app. The Codex IDE extension does not
currently load plugins.

## Antigravity CLI manual install

Generate the distribution first (`dist/` is not committed), then validate and install it with `agy`:
