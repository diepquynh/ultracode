# {{command_prefix}}explore — research one topic
...
**Spawn the prefixed name.** `{{agent_selector}}` is `ultracode:explore` verbatim — the `ultracode:` prefix is part
...
test -f "$REPO_ROOT/{{runtime_dir}}/INVENTORY.md" && echo inventory=ok || echo inventory=MISSING
```

The session ID ({{session_id_names}}) is set by the harness and inherited unchanged
...
3. Tell the user the next stage is `{{command_prefix}}generate-spec`, and that it will pick up this session dir automatically.
