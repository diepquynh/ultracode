# {{command_prefix}}implement — code one phase
...
**Spawn the prefixed name** — `{{agent_selector}}: ultracode:implement`, verbatim.
...
it is the same path `{{command_prefix}}plan` wrote its phase files to earlier in this session — no searching for the newest dir.
...
- **No plan and no instructions** → stop and tell the user to run `{{command_prefix}}plan` first, or to describe the change
  inline.
...
re-spawn `{{command_prefix}}implement` to continue.

On success, tell the user code review is the next gate — run `{{command_prefix}}code-review`. The phase is complete once that
review passes.

Then name what is left, without doing any of it:

- **More phases remain** → `{{command_prefix}}implement` again for the next one. Nothing else runs between phases.
- **This was the last phase** → the code is done. Tests and module docs are **optional closing steps** for after
  every phase, run only if the user wants them: `{{command_prefix}}epa` then `{{command_prefix}}write-test` (then `{{command_prefix}}code-review` for the tests),
  and `{{command_prefix}}module-docs`. Neither is a next step you take on your own.
