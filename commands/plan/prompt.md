# {{command_prefix}}plan — plan from the approved spec
...
**Spawn the prefixed name** — `{{agent_selector}}: ultracode:plan`, verbatim. A bare `plan` resolves to the
...
it is the same path `{{command_prefix}}generate-spec` wrote to earlier in this session — no searching for the newest dir, and no
...
- **No spec file anywhere** → stop and tell the user to run `{{command_prefix}}generate-spec` first. Do **not** plan from a
...
A requirement change at this gate restarts at the spec: re-run `{{command_prefix}}generate-spec` with the change, get the
updated spec approved, then re-run `{{command_prefix}}plan` on it. Never patch a phase file to match a new requirement.

Once approved, tell the user to run `{{command_prefix}}implement` per phase (it picks up phase 1 by default), or to let
